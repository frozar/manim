...
class CoordinateSystem(ThreeDScene):
    CONFIG = {
        "cube_opacity" : 0.8,
        # "cube_opacity" : 1.,
        "cube_colors" : [BLUE],
...
class GroupOfCubeSymmetries(ThreeDScene):
    ...
    def construct(self):
        ...
        # cube = self.get_cube(self.cube_opacity, BLUE_E)

        # print dir(cube)
        # print cube.submobjects

        # print "BEFORE SHIFT"
        # print id(cube)
        # cube = cube.shift(OUT*10)
        # print id(cube)
        # print "AFTER  SHIFT"
        # self.play(ShowCreation(cube), ShowCreation(cube.shift(OUT*10)))
        # self.play(ShowCreation(cube))
        # self.play(ShowCreation(cube.shift(OUT*2)))

        # self.play(Animation(cube))

        # print cube.get_center()
        # print OUT * 2 + cube.get_center()
        # print cube.submobjects[0].points
        # for sub in cube.submobjects:
        #     sub.points += 0.5
        # print cube.submobjects[0].points
        ...
        # self.play(Animation(cube))
        # self.play(Animation(cube.shift(UP + RIGHT)))
        cube = self.get_cube(self.cube_opacity, BLUE)
        # cube.shift(cube.get_center())
        cube.stretch(2, 0)
        cube.stretch(2, 1)
        cube.stretch(2, 2)

        cube.side_length = 2
        print "cube.side_length", cube.side_length
        cube.shift(cube.side_length * (UP + RIGHT + OUT) / 2.0)

        # print "cube.get_corner(DOWN+OUT+LEFT)" , cube.get_corner(DOWN+OUT+LEFT)
        # print "cube.get_corner(DOWN+OUT+RIGHT)", cube.get_corner(DOWN+OUT+RIGHT)
        # print "cube.get_corner(DOWN+IN+LEFT)" , cube.get_corner(DOWN+IN+LEFT)
        # print "cube.get_corner(DOWN+IN+RIGHT)", cube.get_corner(DOWN+IN+RIGHT)
        # print "cube.get_corner(UP+OUT+LEFT)" , cube.get_corner(UP+OUT+LEFT)
        # print "cube.get_corner(UP+OUT+RIGHT)", cube.get_corner(UP+OUT+RIGHT)
        # print "cube.get_corner(UP+IN+LEFT)" , cube.get_corner(UP+IN+LEFT)
        # print "cube.get_corner(UP+IN+RIGHT)", cube.get_corner(UP+IN+RIGHT)
        ...
        # print "number_line.number_to_point(0)", number_line.number_to_point(0)
        # dot = Dot(
        ...
        # # print "dot", dot
        # # arrow = Arrow(dot, color = zero_color)
        # # arrow = Arrow(color = zero_color)
        # arrow = Arrow(dot)

        # # words = TextMobject("Follow zero")
        # # words.next_to(arrow.get_start(), UP)
        # # words.highlight(zero_color)
        ...
        # print self.camera.get_spherical_coords()
        phi, theta, distance = self.camera.get_spherical_coords()
        print "phi, theta, distance", phi, theta, distance

        # self.move_camera(phi + np.pi/16, theta + np.pi/16, distance)
        # self.move_camera(phi - np.pi/8, theta - np.pi/16, distance * 10)
        # self.begin_ambient_camera_rotation(1)

        # self.set_camera_position(phi, theta, 0.0)

        print "self.camera.get_spherical_coords()", self.camera.get_spherical_coords()
        self.move_camera(phi, theta, distance * 10) 
        print "self.camera.get_spherical_coords()", self.camera.get_spherical_coords()

        # self.move_camera(phi - np.pi/6, theta, distance)
        # self.move_camera(phi, theta - np.pi/6, distance)

        # print "self.camera.get_spherical_coords()", self.camera.get_spherical_coords()

        # self.camera.pose_at_angle()


        cube_tmp = cube.deepcopy()
        cube_tmp.gradient_highlight(BLUE_E)
        print "cube_tmp.get_width()", cube_tmp.get_width()
        print "0 cube_tmp.side_length", cube_tmp.side_length
        cube_tmp.scale_in_place(0.5)
        # cube_tmp.side_length *= 0.5
        cube_tmp.side_length = cube_tmp.get_width()
        print "1 cube_tmp.side_length", cube_tmp.side_length
        # print "cube_tmp.get_corner(DOWN+OUT+LEFT)" , cube_tmp.get_corner(DOWN+OUT+LEFT)
        # print "cube_tmp.get_corner(DOWN+OUT+RIGHT)", cube_tmp.get_corner(DOWN+OUT+RIGHT)
        # print "cube_tmp.get_corner(DOWN+IN+LEFT)" , cube_tmp.get_corner(DOWN+IN+LEFT)
        # print "cube_tmp.get_corner(DOWN+IN+RIGHT)", cube_tmp.get_corner(DOWN+IN+RIGHT)
        # print "cube_tmp.get_corner(UP+OUT+LEFT)" , cube_tmp.get_corner(UP+OUT+LEFT)
        # print "cube_tmp.get_corner(UP+OUT+RIGHT)", cube_tmp.get_corner(UP+OUT+RIGHT)
        # print "cube_tmp.get_corner(UP+IN+LEFT)" , cube_tmp.get_corner(UP+IN+LEFT)
        # print "cube_tmp.get_corner(UP+IN+RIGHT)", cube_tmp.get_corner(UP+IN+RIGHT)

        self.remove(cube)

        # IN + DOWN + LEFT
        cube_0 = cube_tmp.deepcopy()
        cube_0.shift(cube_0.side_length * (IN + DOWN + LEFT) / 2.0)
        self.play(Animation(cube_0))

        # IN + UP + RIGHT
        cube_1 = cube_tmp.deepcopy()
        cube_1.shift(cube_1.side_length * (IN + UP + RIGHT) / 2.0)
        self.play(Animation(cube_1))

        # OUT + DOWN + RIGHT
        cube_2 = cube_tmp.deepcopy()
        cube_2.shift(cube_2.side_length * (OUT + DOWN + RIGHT) / 2.0)
        self.play(Animation(cube_2))

        # OUT + UP + LEFT
        cube_3 = cube_tmp.deepcopy()
        cube_3.shift(cube_3.side_length * (OUT + UP + LEFT) / 2.0)
        self.play(Animation(cube_3))

        # # self.dither()
        # # cube2 = cube.copy()
        # cube2 = self.get_cube(self.cube_opacity, BLUE)
        # cube2.scale(2)

        # self.play(ReplacementTransform(cube, cube2))
        # self.play(Animation(cube2, run_time = 1.2))

        # cube_cur = cube2.copy()
        # cube_cur.scale(0.5)

        # # self.play(FadeOut(cube2))
        # # self.play(FadeIn(cube_cur, run_time = 1.2))

        # self.play(FadeOut(cube2), FadeIn(cube_cur, run_time = 1.2))


    def get_cube(self, cube_opacity, fill_color):
        cube = Cube(fill_opacity = cube_opacity, stroke_width = self.stroke_width,
                    side_length = self.side_length, fill_color = fill_color)
